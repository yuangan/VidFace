...
class VideoTestTUFS145KDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(VideoTestTUFS145KDataset, self).__init__()
        self.opt = opt
        self.cache_data = opt['cache_data']
        if self.cache_data:
        ...
        #neighbor_list[0:3] = rs_list[0:3]
        #neighbor_list[4:] = rs_list[3:]
...
class VideoTestIJBCDataset(data.Dataset):
    """Video test dataset for IJBC-Test dataset.
    ...
    def __init__(self, opt):
        super(VideoTestIJBCDataset, self).__init__()
        self.opt = opt
        self.cache_data = opt['cache_data']
        if self.cache_data:
            raise NotImplementedError(
                'cache_data in IJBC-Test dataset is not implemented.')
        self.gt_root, self.lq_root = opt['dataroot_gt'], opt['dataroot_lq']
        self.data_info = {
            'lq_path': [],
            'gt_path': [],
        ...
        for idx, subfolder in enumerate(subfolders):
            ...
            self.data_info['lq_path'].append(lq_paths)
            self.data_info['folder'].append('IJBC')
            self.data_info['idx'].append(f'{idx}/{len(subfolders)}')
            self.data_info['border'].append(0)
...
class VideoTestPretrainedCelebADataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(VideoTestPretrainedCelebADataset, self).__init__()
        self.opt = opt
        self.cache_data = opt['cache_data']
        if self.cache_data:
            raise NotImplementedError(
        ...
        for idx, subfolder in enumerate(subfolders):
            # to img 7
            gt_path = osp.join(self.gt_root, subfolder)
            self.data_info['gt_path'].append(gt_path)
            lq_paths = [
                osp.join(self.lq_root, subfolder)
                for i in neighbor_list
            ]
            self.data_info['lq_path'].append(lq_paths)
            self.data_info['folder'].append('celeba')
            self.data_info['idx'].append(f'{idx}/{len(subfolders)}')
            self.data_info['border'].append(0)
