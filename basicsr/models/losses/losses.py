...
def charbonnier_loss(pred, target, eps=1e-12):
    prt = pred - target
    nan_mask = torch.isnan(prt)
    prt = prt[~nan_mask]
    return torch.sqrt(prt**2 + eps)

@weighted_loss
def offset_loss(pred, target, eps=1e-12):

    # return torch.where(abs(pred - target) >= 5, 1*abs(pred - target), 0 * abs(pred - target)) 
    return torch.where(abs(pred - target) >= 0.1, 1*torch.sqrt((pred - target)**2+eps), 0 * abs(pred - target)) 
...
class OffsetLoss(nn.Module):
    ...
    def forward(self, pred, target, weight=None, **kwargs):
        ...
        """
        return self.loss_weight * offset_loss(
